...
function App() {
  ...
  // The array ensures that the function is not executed after the component rerenders
  useEffect(function() {
...
function Header({showForm, setShowForm}) {
  ...
  return (
    <header className="header">
      ...
      >
    {showForm === true ? 'Close' : 'Share a fact'}</button>
    </header>
  )
}
...
function NewFactForm({ setFacts, setShowForm }) {
  ...
  async function handleSubmit(e) {
    ...
    if(text && isValidHttpUrl(source) && category && text.length <= 200) {
      ...
      // newFact[0] adds fact to the top of the list
  ...
  return (
  <form className="fact-form" onSubmit={handleSubmit}>Fact form
    <input 
      type="text" 
    ...
    <aside>
      <ul>
        ...
        <li key={category.name} className="category">
          <button 
            className="btn btn-category" style={{ backgroundColor: category.color }} onClick={() => setCurrentCategory(category.name)}>
...
function Fact({ fact, setFacts }) {
  ...
  return (
    <li className="fact">
      ...
      <span className="tag" style={{ backgroundColor: CATEGORIES.find((category) => category.name === fact.category).color }}>{fact.category}</span>
